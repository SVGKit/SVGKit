...
#import "SVGKNamedNodeMap_Iterable.h" // Needed for the optional (non-SVG spec) "recursive toXML" method
...
- (id)initType:(DOMNodeType) nt name:(NSString*) n value:(NSString*) v
{
    ...
    if (self) {
		...
		self.childNodes = [[[SVGKNodeList alloc] init] autorelease];
    }
    return self;
}
    ...
    if (self) {
        ...
        switch( nt )
		{
			...
			case DOMNodeType_ELEMENT_NODE:
			{
				...
				self.attributes = [[[SVGKNamedNodeMap alloc] init] autorelease];
			}break;
		}
