...
@implementation SVGKCSSStyleSheet
...
-(long)insertRule:(NSString *)rule index:(unsigned long)index
{
	...
	{
		SVGKCSSStyleRule* newRule = [[[SVGKCSSStyleRule alloc] initWithSelectorText:[stringSplitContainer objectAtIndex:0] styleText:[stringSplitContainer objectAtIndex:1]] autorelease];
...
- (id)initWithString:(NSString*) styleSheetBody
{
    ...
    if (self)
	{
		self.cssRules = [[[SVGKCSSRuleList alloc]init] autorelease];
